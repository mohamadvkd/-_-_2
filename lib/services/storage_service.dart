...
/// - Cache للسور والآيات (بدون إنترنت بعد التحميل الأول)
/// - المفضلة (Bookmarks)
/// - آخر قراءة (Last Read)
/// - الإعدادات (الوضع، حجم الخط، القارئ المفضل)
class StorageService {
  ...
  static Future<void> saveSurahsCache(List<Surah> surahs) async {
    try {
      ...
      await _prefs.setInt(_keySurahsCacheTime, DateTime.now().millisecondsSinceEpoch);
    } catch (_) {}
  }
  ...
  static List<Surah>? getSurahsCache() {
    try {
      ...
      if (now - timeStr > _cacheDurationMs) return null; // انتهت الصلاحية

      final raw = _prefs.getString(_keySurahsCache);
      if (raw == null || raw.isEmpty) return null;

      final List<dynamic> data = jsonDecode(raw) as List<dynamic>;
      return data.map((item) => Surah.fromJson(item as Map<String, dynamic>)).toList();
    } catch (_) {
      return null;
    }
  }
  ...
  // ============================================================
  static Future<void> saveAyahsCache(int surahNumber, List<Ayah> ayahs) async {
    try {
  ...
  static List<Ayah>? getAyahsCache(int surahNumber) {
    try {
      ...
      final List<dynamic> data = jsonDecode(raw) as List<dynamic>;
      return data.map((item) => Ayah.fromJson(item as Map<String, dynamic>)).toList();
    } catch (_) {
      return null;
    }
  }
  ...
  static Future<void> clearAllCache() async {
    ...
    final keys = _prefs.getKeys().where((k) => k.startsWith(_keyAyahsPrefix)).toList();
  ...
  static Future<void> addBookmark({
    ...
    // لا نضيف مكرر
    bookmarks.removeWhere((b) => '${b['surahNumber']}:${b['ayahNumber']}' == key);
